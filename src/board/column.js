...
export default function Column({column, add, onChange, changeTaskName, changeTaskText, changeTaskWorker}){
        ...
        <div className="column">
            ...
            { column.colRaws.map( raw => {
                if (!(raw.rawName === "upupdowndownleftrightleftrightAB")){
                    return <Task task={raw} changeTaskName={changeTaskName} changeTaskText={changeTaskText} changeTaskWorker={changeTaskWorker} colId={column.colId} key={raw.rawId}/>
                }
                else{
